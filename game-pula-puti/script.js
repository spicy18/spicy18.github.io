    ...
    const initialBetMultiplier = 1.2
    ...
    document.addEventListener('DOMContentLoaded', (e) => {
        ...
        if(!localStorage.getItem('PPG-bet-multiplier')) {
            localStorage.setItem('PPG-bet-multiplier', initialBetMultiplier);
        }
    ...
    function spin() {
        ...
        if(day > 30) {
            localStorage.removeItem('PPG-current-month-money')
        ...
        if(!localStorage.getItem('PPG-results')) {
            ...
            if(currentBet > lastResult.currentMoney) {
                dailyEarningsConter(lastResult.currentMoney, false, day);
                day++;
                reset();
                return;
            }
            if(lastResult.currentMoney >= 2000) {
                dailyEarningsConter(lastResult.currentMoney, true, day);
                day++;
                reset();
                return;
            }
    ...
    let greens = 0, reds = 0;
    function dailyEarningsConter(currentMoney, isWinToday, day) {
        if(!localStorage.getItem('PPG-current-month-money')) {
            localStorage.setItem('PPG-current-month-money', 0);
        }
        

        const greensSpan = document.getElementById('greens');
        const redsSpan = document.getElementById('reds');
        let td = document.getElementById(`td-${day}`);

        const thisMonthEarningsLS = localStorage.getItem('PPG-current-month-money');

        if(isWinToday) {

            /*const totalMonthEarnings = parseInt(thisMonthEarningsLS) + (parseInt(thisMonthEarningsLS) + currentMoney);
            localStorage.setItem('PPG-current-month-money', totalMonthEarnings);*/
            ...
            /*const totalMonthEarnings = parseInt(remainingMoneyToday) + (parseInt(remainingMoneyToday) + currentMoney);
            localStorage.setItem('PPG-current-month-money', totalMonthEarnings);*/

            reds++;
            td.style.backgroundColor = '#b90000';
        }
        greensSpan.innerHTML = greens;
        redsSpan.innerHTML = reds;
        
        /*if(parseInt(thisMonthEarningsLS.value) > 0) {
            thisMonthEarnings.style.color = 'green';
            thisMonthEarnings.innerHTML = `+${localStorage.getItem('PPG-current-month-money')}`;
        } else {
            thisMonthEarnings.style.color = '#b90000';
            thisMonthEarnings.innerHTML = `-${localStorage.getItem('PPG-current-month-money')}`;
        }*/
